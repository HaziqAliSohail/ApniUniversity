...
func (o *UpdateClassNameURL) Build() (*url.URL, error) {
	...
	var _path = "/class/{ID}/name"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{ID}", id, -1)
	} else {
		return nil, errors.New("id is required on UpdateClassNameURL")
	}
...
func (o *UpdateClassNameURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on UpdateClassNameURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on UpdateClassNameURL")
	}
