...
func (o *GetTeacherByIDURL) Build() (*url.URL, error) {
	...
	var _path = "/teacher/{ID}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{ID}", id, -1)
	} else {
		return nil, errors.New("id is required on GetTeacherByIDURL")
	}
...
func (o *GetTeacherByIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetTeacherByIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetTeacherByIDURL")
	}
