...
func (o *AssignClassURL) Build() (*url.URL, error) {
	...
	var _path = "/subject/{ID}/class"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{ID}", id, -1)
	} else {
		return nil, errors.New("id is required on AssignClassURL")
	}
...
func (o *AssignClassURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on AssignClassURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on AssignClassURL")
	}
