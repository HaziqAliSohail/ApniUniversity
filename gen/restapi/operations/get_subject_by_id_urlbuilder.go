...
func (o *GetSubjectByIDURL) Build() (*url.URL, error) {
	...
	var _path = "/subject/{ID}"

	id := swag.FormatInt64(o.ID)
	if id != "" {
		_path = strings.Replace(_path, "{ID}", id, -1)
	} else {
		return nil, errors.New("id is required on GetSubjectByIDURL")
	}
...
func (o *GetSubjectByIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetSubjectByIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetSubjectByIDURL")
	}
